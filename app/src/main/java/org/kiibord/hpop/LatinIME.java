...
//            }
    ...
    private void doubleSpace() {
        ...
        CharSequence lastThree = ic.getTextBeforeCursor(3, 0);
        if (lastThree != null && lastThree.length() == 3
                // && Character.isLetterOrDigit(lastThree.charAt(0)) // orijnl
                && voes_or_dijit_kya(lastThree.charAt(0)) // vimql ((c > 0x2f) && (c < 0x40))
                && lastThree.charAt(1) == ASCII_SPACE
    ...
    private void startMultitouchShift() {
        ...
        if (mKeyboardSwitcher.isAlphabetMode()) {
            ...
            if (mSavedShiftState == Keyboard.SHIFT_LOCKED) newState = Keyboard.SHIFT_LOCKED;
        }
    ...
    private void resetMultitouchShift() {
        int newState = Keyboard.SHIFT_OFF;
        if (
//                mSavedShiftState == Keyboard.SHIFT_CAPS_LOCKED ||
...
//        return sKeyboardSettings.capsLock ? Keyboard.SHIFT_CAPS_LOCKED : Keyboard.SHIFT_LOCKED;
    ...
    private void pickSuggestion(CharSequence suggestion, boolean correcting) {
        LatinKeyboardView inputView = mKeyboardSwitcher.getInputView();
        int shiftState = getShiftState();
        if (shiftState == Keyboard.SHIFT_LOCKED
//                || shiftState == Keyboard.SHIFT_CAPS_LOCKED
        ) {
