...
class MiniKeyboardKeyDetector extends KeyDetector {
    ...
    @Override
    public int getKeyIndexAndNearbyCodes(int x, int y, int[] allKeys) { final Key[] keys = getKeys();         final int touchX = getTouchX(x); final int touchY = getTouchY(y);
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            if (dist < closestKeyDist) { closestKeyIndex = i; closestKeyDist = dist; }
        }
